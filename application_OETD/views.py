...
def critery_category(request,_id):
    if request.method == "GET":
        ...
        # value = list(Values.objects.filter(form_id=form.id, field_id=fields.id).all())
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect('critery_category', _id)
        except Exception as e:
